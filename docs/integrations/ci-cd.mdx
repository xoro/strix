...
```bash
strix --non-interactive --target ./app --scan-mode quick
```
...
```bash
strix --non-interactive --target ./app --scan-mode quick --scope-mode diff --diff-base origin/main
```
...
security-scan:
  ...
  script:
    ...
    - strix --non-interactive --target ./ --scan-mode quick
```
    ...
    stages {
        stage('Security Scan') {
            steps {
                ...
                sh 'strix --non-interactive --target ./ --scan-mode quick'
            }
        }
    }
}
...
jobs:
  security-scan:
    ...
    steps:
      ...
      - run:
          name: Run Scan
          command: strix --non-interactive --target ./ --scan-mode quick
```
